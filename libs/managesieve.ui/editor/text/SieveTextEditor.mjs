...
/**
 * CHANGE (remsrc):
 * Added dynamic dark-mode support for CodeMirror 6.
 *
 * - Detects system theme via prefers-color-scheme
 * - Injects a dark theme extension into the editor configuration
 * - Uses extension-based theming (CM6) instead of legacy theme option (CM5)
 *
 * Note:
 * CodeMirror 6 does not support string-based themes; themes must be provided
 * as extensions and explicitly added to the EditorState configuration.
 */
/**
 * CHANGE (remsrc):
 * Introduced local dark theme implementation for CodeMirror 6.
 *
 * Rationale:
 * - Thunderbird extensions do not support bare module specifiers
 *   (e.g. "@codemirror/theme-one-dark")
 * - Avoids dependency on external module resolution or bundling
 *
 * This theme mimics a minimal dark appearance compatible with Bootstrap dark mode.
 */
 /**
 * Modification (2026-04-04, remsrc)
 *
 * Summary:
 * Refactored indentation handling to align with CodeMirror 6.
 *
 * Details:
 * - Replaced legacy indentation model ("indentation-width", "indentation-policy")
 *   with CM6-compatible "indentation-unit".
 * - Implemented setIndentationUnit() using CM6 indentUnit extension.
 * - Updated loadSettings(), loadDefaultSettings(), saveDefaultSettings()
 *   to use "indentation-unit" consistently.
 * - Removed dependencies on deprecated methods:
 *     setIndentWidth(), setIndentWithTabs(), getIndentWithTabs().
 * - Ensured UI mapping (width + tabs/spaces) is converted to indentation-unit.
 *
 * Rationale:
 * Fixes runtime errors:
 *   - "Unknown settings indentation-unit"
 *   - "this.setIndentWidth is not a function"
 * and restores compatibility with Thunderbird ≥128 and CodeMirror 6.
 */
