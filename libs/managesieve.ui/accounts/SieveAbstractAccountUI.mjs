...
class SieveAbstractAccountUI {
  ...
  async renderSettings() {
    ...
    const password = elm.querySelector(".sieve-settings-password");
    const passwordStatus = elm.querySelector(".sieve-settings-password-status");
    password.value = "";
    password.placeholder = account.hasPassword ? "••••••••" : "";
    if (!account.hasPassword) {
      passwordStatus.textContent = "Noch kein Kennwort gespeichert.";
    } else if (account.passwordStorage?.type === "system") {
      passwordStatus.textContent =
        "Kennwort im Passwortspeicher des Betriebssystems gespeichert. " +
        "Nur zum Ändern neu eingeben.";
    } else {
      passwordStatus.textContent =
        "Kennwort lokal in der Add-on-Konfiguration gespeichert. " +
        "Auf diesem System ist derzeit kein unterstützter sicherer " +
        "Passwortspeicher verfügbar.";
    }
    ...
    if (elm.querySelector(".sieve-account-save-settings")) {
      elm.querySelector(".sieve-account-save-settings")
        .addEventListener("click", async () => {
          const hostname = elm.querySelector(".sieve-settings-hostname").value.trim();
          const port = elm.querySelector(".sieve-settings-port").value;
          const username = elm.querySelector(".sieve-settings-username").value.trim();
          const password = elm.querySelector(".sieve-settings-password").value;

          if (!hostname || !username)
            return;

          const settings = { hostname, port, username };
          if (password !== "")
            settings.password = password;

          await this.send("account-set-settings", settings);
          await this.renderSettings();
        });
    }

    if (elm.querySelector(".sieve-account-clear-password")) {
      elm.querySelector(".sieve-account-clear-password")
        .addEventListener("click", async () => {
          await this.send("account-clear-password");
          await this.renderSettings();
        });
    }
